...
def plot_comparison(simu_dir, methods):
    ...
    op_control = np.loadtxt(os.path.join(simu_dir, 'Open_loop_control.txt'))
    ...
    def load_data(method):
        ...
        utilities = config.q[0] * (state[:-1, 0] - ref[:-1, 0]) ** 2 \
                    + config.q[1] * (state[:-1, 2] - ref[:-1, 2]) ** 2 \
                    + config.r * control ** 2
    ...
    for method in methods:
        ...
        for step in range(1):
            ...
            if method.startswith('ADP'):
                ...
                fig_list.append(fig)
                state_predict = []
                virtual_state = state.clone().detach()
                state_predict.append(virtual_state.numpy().squeeze())
                for virtual_step in range(30):
                    virtual_u = policy.forward(preprocessor.torch_preprocess(virtual_state, x_ref[:, -1]))
                    if virtual_step == 0: u = virtual_u.clone().detach()
                    virtual_state, _, _, _, _, _, _ = statemodel_plt.step(virtual_state, virtual_u)
                    # virtual_state_r = virtual_state.detach().clone()[:, 0:4] - x_ref
                    state_predict.append(virtual_state.detach().numpy().squeeze())
                    action_list.append(virtual_u.detach().numpy().squeeze())
                state_predict = np.array(state_predict)
                label = 'ADP'
                marker = 'D'
                ms_size = 2.0
                ...
                x = state.tolist()[0]
                state_predict, action_list = solver.mpcSolver(x, pred_steps)
                ...
                continue
            time = 0.1 * np.arange(len(state_predict[:, -1] - state_predict[0, -1]))
            state_predict[:, -1] = state_predict[:, -1] - state_predict[0, -1]
            data_x = 100 * state_predict[:, index_x] if index_x == 0 else time
            if index_y == 2:
                data_y = 180 / np.pi * state_predict[:, index_y]
            elif index_y == -1:
                data_y = action_list
                data_x = data_x[:-1]
            else:
                data_y = state_predict[:, index_y]
