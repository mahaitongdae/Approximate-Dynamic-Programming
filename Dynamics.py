...
class VehicleDynamics(DynamicsConfig):
    ...
    def _state_function(self, state, control):
        ...
        # # cornering force of front and rear angle, Fiala tire model
        # self.Ff_w1 = pow(self.k1, 2) / (3 * self.u * self.F_z1)
        # self.Fr_w1 = pow(self.k2, 2) / (3 * self.u * self.F_z2)
        #
        # self.Ff_w2 = pow(self.k1, 3) / (27 * pow(self.u * self.F_z1, 2))
        # self.Fr_w2 = pow(self.k2, 3) / (27 * pow(self.u * self.F_z2, 2))
        #
        # # F_yf_max=Fzf * u
        # self.F_yf_max = torch.tensor(self.F_z1 * self.u, dtype=torch.float32)
        # self.F_yr_max = torch.tensor(self.F_z2 * self.u, dtype=torch.float32)
        #
        # F_y1 = - self.k1 * torch.tan(alpha_1) + self.Ff_w1 * torch.mul(torch.tan(alpha_1), torch.abs(
        #     torch.tan(alpha_1))) - self.Ff_w2 * torch.pow(torch.tan(alpha_1), 3)
        # F_y2 = - self.k2 * torch.tan(alpha_2) + self.Fr_w1 * torch.mul(torch.tan(alpha_2), torch.abs(
        #     torch.tan(alpha_2))) - self.Fr_w2 * torch.pow(torch.tan(alpha_2), 3)
        #
        # F_y1[F_y1 > self.F_yf_max] = self.F_yf_max
        # F_y1[F_y1 < -self.F_yf_max] = -self.F_yf_max
        #
        # F_y2[F_y2 > self.F_yr_max] = self.F_yr_max
        # F_y2[F_y2 < -self.F_yr_max] = -self.F_yr_max
    ...
    def utility(state, control):
        ...
        utility: tensor   shape: [BATCH_SIZE, ]
            utility, i.e. l(x,u)
        """
        utility = 0.1 * (10 * torch.pow(state[:, 0], 2) + 10 * torch.pow(state[:, 2], 2) + 10 * torch.pow(control[:, 0], 2))
        return utility
...
if __name__ == '__main__':
    ...
    x = torch.range(0,300,0.1)
    plt.figure()
    plt.plot(x, dynamics.referenceTrajectoryDlc(x))
    # print(dynamics.referenceTrajectoryDlc(x))
    plt.show()
